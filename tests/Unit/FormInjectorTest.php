...
final class FormInjectorTest extends TestCase
{
    ...
    public function test_inject_into_form_with_recaptcha_inserts_before_recaptcha_info(): void
    {
        $html = '<form>'
            ...
            . '<span class="recaptcha-info">This site is protected by reCAPTCHA</span>'
            . '</div>'
        ...
        $consent_pos  = strpos( $out, 'mautic_consent' );
        $recaptcha_pos = strpos( $out, 'recaptcha-info' );
        $submit_pos   = strpos( $out, 'elementor-field-type-submit' );

        $this->assertNotFalse( $consent_pos );
        $this->assertNotFalse( $recaptcha_pos );
        $this->assertNotFalse( $submit_pos );
